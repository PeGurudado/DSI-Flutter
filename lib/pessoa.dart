...
class PessoaController {
  ...
  Future<void> remove(FutureOr<Pessoa> pessoa) async {
    ...
    Query queryB = FirebaseFirestore.instance
        .collection("professores")
        .where("pessoa", isEqualTo: getRef(p.id));
    QuerySnapshot snapshotB = await queryB.get();

    if (snapshot.size > 0)
      throw Exception(
          'Não é possível remover a pessoa, pois existe um aluno associado '
          'a ela. Caso deseje, remova o aluno.');
    else if (snapshotB.size > 0)
      throw Exception(
          'Não é possível remover a pessoa, pois existe um professor associado '
              'a ela. Caso deseje, remova o professor.');
    else
